...
struct background
{
  ...
  /** @name - Dark Geometry parameters (Hertault model) */

  //@{

  short has_dg;              /**< presence of Dark Geometry? */
  double dg_alpha_star;      /**< DG coupling alpha* ~ 0.075 */
  double dg_rho_c;           /**< DG critical density rho_c [kg/m^3] */
  double dg_beta;            /**< DG exponent beta = 2/3 */
  double dg_k_suppression;   /**< suppression scale k_s [h/Mpc] */
  double dg_beta_suppression;/**< suppression slope */
  double dg_A_suppression;   /**< suppression amplitude */
  double dg_z_transition;    /**< transition redshift (computed) */

  /* DG-E Extended parameters for H0 tension */
  short has_dg_extended;     /**< presence of DG-Extended? */
  double dg_xi_0;            /**< non-minimal coupling xi_0 ~ 0.10-0.15 */
  double dg_beta_xi;         /**< running of xi: xi(z) = xi_0 + beta_xi*ln(1+z) */
  double dg_beta_alpha;      /**< running of alpha: alpha(z) = alpha_0*(1 + beta_alpha*ln(1+z)) */
  double dg_eta;             /**< efficiency factor for H0 correction */

 ...
 * * Dark Geometry extension by H. Hertault in 2025
 *
...
int background_functions(
  ...
  /* DG-E correction: non-minimal coupling xi*R*phi^2 modifies H^2 at high z */
  double dg_H_correction = 1.0;
  if (pba->has_dg_extended == _TRUE_) {
    double z = 1.0/a - 1.0;
    /* The non-minimal coupling xi*R*phi^2 adds to the effective density.
       At z ~ 1000, R ~ 3*H^2*(1+w_eff) with w_eff ~ 1/3 (radiation).
       The effect is to increase H at high z, which reduces the sound horizon.
       
       Calibration from HCM_Extended_numerical.py:
       xi_0 ~ 0.10-0.15 gives H0 = 71-74 km/s/Mpc
       The correction is: H_DGE/H_LCDM = sqrt(1 + delta)
       where delta = f_EDE * (effective fraction of early dark energy)
       
       We model this as an effective EDE-like contribution at z > 100 */
    if (z > 10.0) {
      double xi_z = pba->dg_xi_0 + pba->dg_beta_xi * log(1.0 + z);
      /* Correction peaks around z ~ 1000 and decays at late times */
      double z_peak = 1000.0;
      double width = 500.0;
      double gaussian = exp(-0.5 * pow((z - z_peak)/width, 2));
      /* Effective fractional increase in H^2 */
      double f_ede_eff = 0.10 * xi_z * gaussian;  /* ~10% effect for xi=0.1 */
      dg_H_correction = sqrt(1.0 + f_ede_eff);
    }
  }

  pvecback[pba->index_bg_H] = sqrt(rho_tot-pba->K/a/a) * dg_H_correction;

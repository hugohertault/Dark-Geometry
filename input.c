...
int input_read_parameters_species(struct file_content * pfc,
  ...
  /** 10) Dark Geometry parameters (Hertault model) */
  class_call(parser_read_string(pfc,"has_dg",&string1,&flag1,errmsg),
             errmsg,
             errmsg);
  ...
  if (pba->has_dg == _TRUE_) {
    class_read_double("dg_alpha_star",pba->dg_alpha_star);
    class_read_double("dg_rho_c",pba->dg_rho_c);
    class_read_double("dg_beta",pba->dg_beta);
    class_read_double("dg_k_suppression",pba->dg_k_suppression);
    class_read_double("dg_beta_suppression",pba->dg_beta_suppression);
    class_read_double("dg_A_suppression",pba->dg_A_suppression);

    /* Compute transition redshift from parameters */
    double H0_SI = pba->H0 * _c_;
    double rho_crit_0 = 3.0 * H0_SI * H0_SI / (8.0 * _PI_ * _G_);
    double rho_m_0 = (pba->Omega0_cdm + pba->Omega0_b) * rho_crit_0;
    if (rho_m_0 > 0 && pba->dg_rho_c > 0) {
      pba->dg_z_transition = pow(pba->dg_rho_c / rho_m_0, 1.0/3.0) - 1.0;
      if (pba->dg_z_transition < 0) pba->dg_z_transition = 0;
    }

    if (pba->background_verbose > 0) {
      printf(" -> Dark Geometry enabled with alpha* = %g, z_trans = %.2f\n",
             pba->dg_alpha_star, pba->dg_z_transition);
    }
  }

  /* DG-E Extended for H0 tension */
  class_call(parser_read_string(pfc,"has_dg_extended",&string1,&flag1,errmsg),
             errmsg,
             errmsg);
  if ((flag1 == _TRUE_) && ((strstr(string1,"y") != NULL) || (strstr(string1,"Y") != NULL))) {
    pba->has_dg_extended = _TRUE_;
    pba->has_dg = _TRUE_;  /* DG-E implies DG */
  }

  if (pba->has_dg_extended == _TRUE_) {
    class_read_double("dg_xi_0",pba->dg_xi_0);
    class_read_double("dg_beta_xi",pba->dg_beta_xi);
    class_read_double("dg_beta_alpha",pba->dg_beta_alpha);
    class_read_double("dg_eta",pba->dg_eta);

    if (pba->background_verbose > 0) {
      printf(" -> DG-Extended enabled with xi_0 = %g, beta_alpha = %g\n",
             pba->dg_xi_0, pba->dg_beta_alpha);
    }
  }
...
int input_default_params(struct background *pba,
  ...
  /** 10) Dark Geometry parameters (Hertault model) */
  pba->has_dg = _FALSE_;
  pba->dg_alpha_star = 0.075;
  pba->dg_rho_c = 5.82e-27;
  pba->dg_beta = 2.0/3.0;
  pba->dg_k_suppression = 0.1;
  pba->dg_beta_suppression = 2.8;
  pba->dg_A_suppression = 0.25;
  pba->dg_z_transition = 0.3;

  /* DG-E Extended parameters */
  pba->has_dg_extended = _FALSE_;
  pba->dg_xi_0 = 0.12;
  pba->dg_beta_xi = 0.02;
  pba->dg_beta_alpha = 0.04;
  pba->dg_eta = 80.0;

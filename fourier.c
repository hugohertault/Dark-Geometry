...
int fourier_pk_linear(
  ...
  for (index_k=0; index_k<k_size; index_k++) {
    ...
    /** --> Apply Dark Geometry suppression if enabled */
    if (pba->has_dg == _TRUE_) {
      double k = exp(pfo->ln_k[index_k]);  /* k in 1/Mpc */
      double k_h = k * pba->h;              /* k in h/Mpc */
      double x = k_h / pba->dg_k_suppression;
      double f = 1.0 / (1.0 + pow(x, pba->dg_beta_suppression));
      double S = 1.0 - pba->dg_A_suppression * (1.0 - f);
      lnpk[index_k] += log(S);
    }
  }
